...
function Jobs(props) {
    ...
    const [filters, setFilters] = useState({"category": "any", "location": "any", "searchCriteria": "position"})
    ...
    return (
        <div style={{ paddingTop: "80px" }}>
            ...
            {jobPosts.map((data, index) => {
                ...
                if(
                    (
                        (filters.category == job_c[data['job_category']] && filters.location == data['location'].toLowerCase()) ||
                        (filters.category == "any" && filters.location == "any") ||
                        (filters.category == "any" && filters.location == data['location'].toLowerCase()) ||
                        (filters.category == job_c[data['job_category']] && filters.location == "any")
                    ) &&
                    (filters.searchCriteria == "position" ? data['job_title'].toLowerCase().includes(search) : data['recruiter_id']['company_name'].toLowerCase().includes(search) )
                )
                    return (
